...
export default function AgentCallPage() {
  ...
  const createNewRoom = async () => {
    ...
    try {
      ...
      setStatusMessage("Error accessing microphone. Please check your permissions.")
      setIsAgentAvailable(false)
      toast({
      ...
      peerConnectionRef.current.ontrack = (event) => {
        ...
        if (remoteStreamRef.current) {
          ...
          if (remoteStreamRef.current.getAudioTracks().length > 0) {
            ...
            if (!transcriptionRecorderRef.current && remoteStreamRef.current.getAudioTracks().length > 0) {
    ...
    try {
      console.log("Creating Firestore room document...")
      ...
      if (peerConnectionRef.current) {
        peerConnectionRef.current.onicecandidate = (event) => {
          if (event.candidate) {
            console.log("New ICE candidate for agent:", event.candidate.candidate.substring(0, 50) + "...")
      ...
      if (peerConnectionRef.current) {
        ...
        console.log("Storing room data in Firestore...")
        ...
        console.log("Setting up room snapshot listener...")
        const unsubscribe = onSnapshot(
          roomRef,
          async (snapshot) => {
            const data = snapshot.data()
            if (!peerConnectionRef.current) {
              console.log("Peer connection no longer exists, ignoring room update")
              return
            }
            ...
            if (!peerConnectionRef.current.currentRemoteDescription && data && data.answer) {
              ...
              console.log("Received answer from client, establishing connection...")
            ...
            if (data && data.status === "ended" && peerConnectionRef.current) {
              console.log("Client ended the call")
          ...
          (snapshot) => {
            snapshot.docChanges().forEach((change) => {
              if (change.type === "added") {
                ...
                console.log("Received ICE candidate from client:", candidate.candidate.substring(0, 50) + "...")
                peerConnectionRef.current
                  ?.addIceCandidate(candidate)
                  .catch((err) => console.error("Error adding ICE candidate from client:", err))
              }
            })
          },
          (error) => {
  ...
  const validateRoomStatus = async (roomId: string) => {
    ...
    try {
      console.log("Validating room status for room:", roomId)
      ...
      if (roomData.createdAt) {
        const roomAge = new Date().getTime() - roomData.createdAt.toDate().getTime()
  ...
  const endCurrentSession = () => {
    ...
    if (callStartTimeRef.current) {
      callDurationSeconds = Math.floor((new Date().getTime() - callStartTimeRef.current.getTime()) / 1000)
    ...
    if (roomId && firestoreRef.current) {
      console.log("Updating room status in Firestore...")
      const roomRef = doc(firestoreRef.current, "rooms", roomId)
      ...
      console.log("Cleaning up ICE candidates...")
      ...
      getDocs(callerCandidatesRef).then((snapshot) => {
        snapshot.forEach((document) => {
          ...
          deleteDoc(doc(firestoreRef.current, "rooms", roomId, "callerCandidates", document.id)).catch((err) =>
      ...
      getDocs(calleeCandidatesRef).then((snapshot) => {
        snapshot.forEach((document) => {
          // Use deleteDoc with the document reference instead of doc.ref.delete()
          deleteDoc(doc(firestoreRef.current, "rooms", roomId, "calleeCandidates", document.id)).catch((err) =>
  ...
  const startTranscriptionRecorder = () => {
    ...
    const createAndStartRecorder = () => {
      ...
      try {
        transcriptionRecorderRef.current = new MediaRecorder(remoteStreamRef.current, options)
      } catch (e) {
        ...
        if (event.data && event.data.size > 0) {
          ...
          reader.onloadend = async () => {
            ...
            try {
              ...
              const response = await transcribeAudio(base64data, roomId, statusMessage)
              if (response.success !== false) {
                console.log("Transcription received:", response.text.substring(0, 50) + "...")
  ...
  const startFullRecorder = () => {
    ...
    try {
      ...
      const options = { mimeType: "audio/webm" }
      try {
        fullRecorderRef.current = new MediaRecorder(destination.stream, options)
      } catch (e) {
      ...
      fullChunksRef.current = []
      const recordingRoomId = roomId // Store the roomId at recording start
      ...
      fullRecorderRef.current.onstop = () => {
        ...
        reader.onloadend = async () => {
          ...
          try {
            ...
            const response = await saveConversation(base64data, recordingRoomId)
  ...
  const sendCallDataToBackend = async () => {
    ...
    try {
      ...
      await logCallData({
        agentUsername: "agent",
        callDuration: callDurationSeconds,
        callStart: callStartTimeRef.current ? callStartTimeRef.current.toISOString() : null,
        roomId: roomId,
      })
      console.log("Call data sent successfully")
    } catch (err) {
  ...
  return (
    <div className="flex-1 space-y-4 p-8 pt-6">
      <h1 className="text-3xl font-bold tracking-tight">Agent Call Interface</h1>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                >
                  {isInitializing ? "Initializing..." : isAgentAvailable ? "Disconnect" : "Connect"}
                </Button>
                <p className="mt-2">
            ...
            {transcription.length > 0 && (
              <div className="mt-6">
                <h2 className="text-xl font-semibold mb-2">Transcription (Caller Audio):</h2>
